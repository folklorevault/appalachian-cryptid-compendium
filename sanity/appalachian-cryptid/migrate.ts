/**
 * Migration script to copy cryptid data from static files to Sanity CMS
 *
...
async function migrate() {
  if (!process.env.SANITY_TOKEN) {
    ...
    console.error('1. Go to https://www.sanity.io/manage/project/8thljucm/api#tokens')
    console.error('2. Create a new token with "Editor" permissions')
    console.error('3. Run: SANITY_TOKEN=your-token-here npx tsx migrate.ts')
    process.exit(1)
  }

  console.log('🦇 Starting Appalachian Cryptid migration to Sanity...')
  console.log('')
  ...
  for (const cryptid of cryptids) {
    ...
    try {
      ...
      console.error(`❌ Failed: ${cryptid.name}`, error)
    }
  }
  ...
  console.log('1. Visit https://cryptid.sanity.studio/ to see your cryptids')
  console.log('2. Upload images for each cryptid in the Studio')
}
